...
<div
  class="border rounded-lg p-4 shadow-sm bg-white relative {loading
    ? 'opacity-70'
    : ''}"
>
  {#if error}
    <div
      class="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded relative mb-4"
      role="alert"
    >
    ...
    <div class="flex items-start">
      ...
      <div class="ml-3 flex-grow">
        <a href={`/tasks/${task.id}`} class="inline-block">
          <h3
            class="text-lg font-medium text-gray-900 {task.completed
        ...
        <p
          class="mt-1 text-gray-600 {task.completed
        ...
        <div class="mt-2 flex flex-wrap items-center gap-2">
          ...
          <span
            class={`text-xs font-medium px-2.5 py-0.5 rounded-full ${getPriorityColor(task.priority)}`}
          >
            {task.priority.charAt(0).toUpperCase() + task.priority.slice(1)}
          </span>

          <span
            class={`text-xs font-medium px-2.5 py-0.5 rounded-full ${task.completed ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800'}`}
          >
  ...
  {#if loading && !isEditing}
    ...
    >
      <div
        class="w-5 h-5 border-2 border-blue-600 rounded-full animate-spin border-t-transparent"
      ></div>
    </div>
  {/if}
</div>
